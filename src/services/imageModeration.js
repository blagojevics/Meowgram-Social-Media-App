...
export const moderateImage = async (imageUrl) => {
  try {
    ...
    return {
      isAllowed: true,
      reason:
        "Basic client-side moderation passed. Upload and let the community help moderate!",
      confidence: 0.8,
    };
  } catch (error) {
 ...
 */
export const checkAnimalContent = (filename, userDescription) => {
  ...
  return {
    isAllowed: true,
    reason:
      "Upload allowed. If this isn't animal-related, the community can help moderate it!",
    confidence: 0.5,
  };
};
 ...
 */
export const getModerationMessage = (result) => {
  if (result.isAllowed) {
    if (result.detectedKeywords && result.detectedKeywords.length > 0) {
      return `🎉 Perfect! We love seeing ${result.detectedKeywords.join(
        ", "
      )} on Meowgram!`;
    }
